...
set :application, "pi-app"

set :scm, :git
set :repository,  "git@github.com:clivebaker/pi-app.git"
...
#    run "sudo RAILS_ENV=#{rails_env} #{File.join(current_path,"script","delayed_job")} restart"
 ...
 # after 'deploy:bundle_install','deploy:create_symlink'
#  after 'deploy:create_symlink','deploy:assets'
#  before "deploy:assets","deploy:symlink"  
#  before "deploy:migrate","deploy:symlink"  
